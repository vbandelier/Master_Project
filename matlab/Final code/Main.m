...
%%  Merton
param_mer = [0.0622    0.1807   -0.0450    0.1320]; % Calibrated
%% Calibration %
% Nx = 500;
% lb = [0 0 -Inf 0];
% ub = [Inf Inf Inf Inf];
...
%%  Kou
param_kou = [0.0623    0.2351    0.1520    8.6911   11.9560]; %0Calibrated
%% Calibration %
% Nx = 500;
% lb = [0 0 0 0 0];
...
%%  NIG
param_nig = [18.6526   -3.8872    0.1188]; % Calibrated
%% Calibration %
% Nx = 500;
% lb = [0  -Inf  0];
% ub = [Inf Inf Inf];
...
%%  VG
param_vg = [-0.0294    0.0796    0.2788]; % Calibrated
...
% TARN_Conv_BS_ExP.set_price(model1,method3);
% disp(strcat('TARN price with Conv= ',num2str(TARN_Conv_BS_ExP.price)));
% %
% disp('---------- Merton -----------------')
