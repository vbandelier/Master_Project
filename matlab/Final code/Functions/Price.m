...
period = option.Period; T = period*N;
...
elseif method.name(1) == 'F'
    ...
    Payoff_fun = @(s,a)Cgain(s,a)+Closs(s,a);
    
    ext_fun = @(x,a,t) (a+Payoff_fun(S0*exp(x+r*t),a))*exp(-r*t);
    ...
    for k = 1:N
        ...
        for j = 1:Na
            ...
            for tn = 1:Nt
                tnew = tn*dt;
                Vleft = ext_fun(quad_grid_left,A(j),tnew);
                Vright= ext_fun(quad_grid_right,A(j),tnew);
                iter = 0;
                ...
                while max(abs(Vstar_old-Vstar))>tol && iter<fix_pt_iter
                    ...
                    fnew(1)  = 0.5*sigma2*ext_fun(Xmin,A(j),tnew)/(dx^2)...
                        - (r-q-0.5*sigma2-c)*ext_fun(Xmin,A(j),tnew)/(2*dx);
                    fnew(end)= 0.5*sigma2*ext_fun(Xmax,A(j),tnew)/(dx^2)...
                        + (r-q-0.5*sigma2-c)*ext_fun(Xmax,A(j),tnew)/(2*dx);
                ...
                V(:,tn+1)=[ext_fun(Xmin,A(j),tnew); sol; ext_fun(Xmax,A(j),tnew)];
            end
            U(:,j) = V(:,end);
        end
    end
